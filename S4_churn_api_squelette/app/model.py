...
def add_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # tenure_group: bin tenure into new (<12) / mid (12-36) / loyal (>=36)
    # MUST match S2: bins=[-1, 12, 36, 1000], labels new/mid/loyal
    ...
    # services_count: number of "Yes" across the premium service columns
    df["services_count"] = (df[SERVICE_COLS] == "Yes").sum(axis=1)

    # has_internet: 1 if the client has internet, else 0
    df["has_internet"] = (df["InternetService"] != "No").astype(int)

    # avg_charge_per_month: protected against tenure=0
